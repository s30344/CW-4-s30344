...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = 
            emps.Where(e => depts.Any(d => d.Loc == "CHICAGO" && d.DeptNo == e.DeptNo)).ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.Join(grades, e=>true,s=>true,(e,s)=>new {Emp = e, Grade = s})
            .Where(x =>x.Emp.Sal>=x.Grade.Losal && x.Emp.Sal <= x.Grade.Hisal)
            .Select(x=>new{x.Emp.EName,x.Grade.Grade}).ToList();
        //
